...
支持 Ctrl / Shift / Alt 的组合键、也支持鼠标键（含侧键），一个组合只能属于一个 App。已经被别的 App 或别的
...
### 玩家可以把手机整个放大，你不用管（1.9.3 起）

「设置 → 界面大小」能把整个手机缩放到 75%–300%。**你的页面什么都不用改**：那是渲染时
套的一层 pose 缩放，`PhoneCanvas` 给的 x/y/宽高，以及送进 `mouseClicked` / `mouseScrolled`
的鼠标坐标，全都在同一套没缩放的坐标系里——两边一起换算过了。

两种写法会出事：

- **自己去问 `Minecraft.getInstance().mouseHandler` 拿鼠标位置。** 那是屏幕原始坐标，没有
  换算，放大之后就对不上。要鼠标位置就用回调里给你的那两个数。
- **自己调 `canvas.graphics().enableScissor(...)`。** 原版那句收窗口坐标，而且【不看
  PoseStack】——你交的是没缩放的手机坐标，裁剪框就停在没缩放时的位置和大小上，文字被切掉
  一块，只在倍数不是 100% 时出现。要裁剪用 **`canvas.clipped(x, y, w, h, () -> { ... })`**，
  它替你把矩形过一遍当前的变换矩阵，缩放几层都对。

`clipped` 只有这一种作用域写法、没有配对的 enable/disable，是故意的：裁剪是全局状态，
enable 之后没能走到 disable 的话，这一帧剩下的所有东西都会被切在你那个框里——你的页面抛
一次异常，玩家看到的是整个游戏界面缺一块。`clipped` 的 body 抛了照样会把裁剪收回去。
